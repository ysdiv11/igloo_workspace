...
const EVENING_TIME_SLOTS = ['19:00', '20:00', '21:00', '22:00', '23:00', '00:00', '01:00'];
...
const App = () => {
  ...
  const [dragIsEvening, setDragIsEvening] = useState(false);
  ...
  const handleMouseDown = (day, slotIndex, hasAcademicBlock = false, isEvening = false) => {
  ...
  const handleMouseUp = () => {
    if (isDragging && dragDay && dragStart !== null && dragEnd !== null) {
      ...
      const slotsArray = dragIsEvening ? EVENING_TIME_SLOTS : TIME_SLOTS;

      const startTime = slotsArray[startSlot];
      const endHour = (parseInt(slotsArray[endSlot].split(':')[0]) + 1) % 24;
  ...
  // Evening slot helpers (19:00 – 01:00)
  const getClassAtSlotEvening = (day, slotIndex) => {
    let schedule;
    if (day === 'Saturday' || day === 'Sunday') {
      schedule = weekendSchedule[day] || [];
    } else {
    ...
    const slotTime = EVENING_TIME_SLOTS[slotIndex];
    const slotHour = parseInt(slotTime.split(':')[0]);
    // Treat 00:00 and 01:00 as 24:00 and 25:00 for comparison
    const slotMinutes = (slotHour < 12 ? slotHour + 24 : slotHour) * 60;

    for (const item of schedule) {
      const startHour = parseInt(item.time.split(':')[0]);
      const endHour = parseInt(item.end.split(':')[0]);
      const startMinutes = (startHour < 12 ? startHour + 24 : startHour) * 60 + parseInt(item.time.split(':')[1]);
      const endMinutes = (endHour < 12 ? endHour + 24 : endHour) * 60 + parseInt(item.end.split(':')[1]);
  ...
  const hasAcademicBlockAtSlotEvening = (day, slotIndex) => {
    if (day === 'Saturday' || day === 'Sunday') return false;
    const academic = academicSchedule[day] || [];
    const slotTime = EVENING_TIME_SLOTS[slotIndex];
    const slotHour = parseInt(slotTime.split(':')[0]);
      ...
      <div className="command-center">
        ...
        <header className="header">
          <div className="header-left">
            ...
            <h1 className="title">Igloo Workspace</h1>
          </div>
          ...
          <AnimatePresence mode="wait">
            {activeView === 'dashboard' ? (
              ...
              >
                ...
                {/* Evening / Night Section Header */}
                <div className="evening-section-header">
                  <Moon size={18} />
                  <span>Evening / Night</span>
                </div>

                {/* Evening Timetable Grid (19:00 – 02:00) */}
                <div
                  className="timetable-grid"
                  onMouseUp={handleMouseUp}
                  onMouseLeave={handleMouseUp}
                >
                  ...
                  {/* Evening Time Slots & Cells */}
                  {EVENING_TIME_SLOTS.map((slot, slotIndex) => (
                    <React.Fragment key={slot}>
                      <div className="timetable-time-cell">{slot}</div>
                      {DAYS.map(day => {
                        const classInfo = getClassAtSlotEvening(day, slotIndex);
                        const isWeekend = day === 'Saturday' || day === 'Sunday';
                        const hasAcademic = hasAcademicBlockAtSlotEvening(day, slotIndex);
                        const isDragTarget = isDragging && dragIsEvening && dragDay === day &&
                          slotIndex >= Math.min(dragStart, dragEnd) &&
                          slotIndex <= Math.max(dragStart, dragEnd);
                        ...
                        return (
                          <div
                            ...
                            className={`timetable-cell ${classInfo ? 'has-class' : ''} ${isWeekend ? 'weekend' : ''} ${isDragTarget ? 'drag-target' : ''} ${hasAcademic ? 'has-academic' : ''}`}
                            style={classInfo?.span > 1 ? { gridRow: `span ${classInfo.span}` } : {}}
                            onMouseDown={() => !classInfo && handleMouseDown(day, slotIndex, hasAcademic, true)}
                            onMouseMove={() => handleMouseMove(slotIndex)}
                            onContextMenu={(e) => classInfo && handleContextMenu(e, day, classInfo.id)}
                            onDoubleClick={(e) => classInfo && handleBlockDoubleClick(e, day, classInfo)}
                          >
